...
class Document(object):
    ...
    def create_intersections(self):
        ...
        for uiu in self.uiu.values():
            ...
            if index != -1:
                ...
                try:
                    uiu = Uiu.objects.get(foglio=u['fg'], numero=u['num'], tipologia='PARTICELLA')
                except:
                    uiu = None
                if uiu:
                    intersect_object = {}
                    ...
                    for model in urbanistic_models:
                        intersecting_polygons = model.objects.filter(the_geom__intersects=uiu.the_geom)
                        for i in intersecting_polygons:
                            intersection = {}
                            intersection['intersection_area'] = i.the_geom.intersection(uiu.the_geom).area
                            intersection['intersection_percentage'] = (intersection['intersection_area'] * 100) / intersect_object['area']
                            intersection['urbanistic_plan'] = re.sub('urbutm$', '', model.__name__)
    ...
    def create_html_document(self):
        ...
        if len(self.intersections) > 0:
            ...
            #personal_data
            text = text.replace('{protocollo}', self.personal_data['num_protocollo'])
            text = text.replace('{data_protocollo}', self.personal_data['data_protocollo'])
            text = text.replace('{richiesta}', self.personal_data['num_richiesta'])
            text = text.replace('{data_richiesta}', self.personal_data['data_richiesta'])
            text = text.replace('{cognome}', self.personal_data['cognome'])
            text = text.replace('{nome}', self.personal_data['nome'])
            text = text.replace('{luogo_nascita}', self.personal_data['luogo_nascita'])
            text = text.replace('{prov_nascita}', self.personal_data['prov_nascita'])
            text = text.replace('{data_nascita}', self.personal_data['data_nascita'])
            ...
            if len(i['intersections']) > 0:
                list_intersections += '<p>che il foglio '+i['fg']+', numero '+i['num']+' <b>('+str(round(i['area'], 2))+'mq)</b> &egrave; incluso nel piano urbanistico:<br><ol>'
                plans = []
                ...
                for plan in plans:
                    ...
                    for z in i['intersections']:
                        if z['urbanistic_plan'] == plan:
                            list_intersections += '<li>per <b>' + str(round(z['intersection_area'], 2)) + ' mq</b> nella zona ' + z['urbanistic_zone']+'</li>'
